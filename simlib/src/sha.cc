...
InplaceBuff<64> sha3_256(StringView str) {
	...
	FIPS202_SHA3_256(reinterpret_cast<const unsigned char*>(str.data()),
	                 str.size(), out);
...
InplaceBuff<96> sha3_384(StringView str) {
	...
	FIPS202_SHA3_384(reinterpret_cast<const unsigned char*>(str.data()),
	                 str.size(), out);
...
InplaceBuff<128> sha3_512(StringView str) {
	...
	FIPS202_SHA3_512(reinterpret_cast<const unsigned char*>(str.data()),
	                 str.size(), out);
