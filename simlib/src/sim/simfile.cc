...
static void append_limits_value(string& res, const sim::Simfile& simfile) {
    ...
    for (const sim::Simfile::TestGroup& group : simfile.tgroups) {
        ...
        for (const sim::Simfile::Test& test : group.tests) {
            string line(concat_tostr(test.name, ' ', to_string(test.time_limit)));
...
string Simfile::dump() const {
    ...
    for (const TestGroup& group : tgroups) {
        for (const Test& test : group.tests) {
            if (!test.in.empty() and (interactive or !test.out.value().empty())) {
...
void Simfile::load_solutions() {
    ...
    for (const string& str : var.as_array()) {
        // Secure path, so that it is not going outside the package
        solutions.emplace_back(path_absolute(str).erase(0, 1)); // Erase '/' from the
    }
    // beginning
}
...
void Simfile::load_global_memory_limit_only() {
    ...
    if (ml.is_set()) {
                        ...
                        20)>(ml.as_string()))
        {
            ...
            throw std::runtime_error{"Simfile: too big value of the `memory_limit`"};
        }

        // Convert from MiB to bytes
        auto gml = ml.as<decltype(global_mem_limit)::value_type>().value() << 20;
        if (gml <= 0) {
            throw invalid_mem_limit();
        }
...
Simfile::parse_limits_item(StringView item) {
    ...
    if (tl <= 0) {
        throw std::runtime_error{concat_tostr("Simfile: invalid time limit of the test `",
                test_name, "` - it has to be grater than 0")};
    }
    ...
    if (time_limit == 0ns) {
        throw std::runtime_error{concat_tostr("Simfile: time limit of the test `", test_name,
    ...
    constexpr auto max_mem_limit = std::numeric_limits<MemLimitType>::max() >> 20;
...
std::tuple<StringView, int64_t> Simfile::parse_scoring_item(StringView item) {
    ...
    if (gid == "\"\"" or gid == "''") {
        ...
        throw std::runtime_error{concat_tostr("Simfile: scoring of the invalid group `", gid,
        ...
        if (not memory_limit.has_value()) {
            if (not global_mem_limit.has_value()) {
                throw std::runtime_error{concat_tostr(
                        "Simfile: missing memory limit for the test `", test_name, '`')};
            }
...
void Simfile::load_tests_files() {
    ...
    std::map<StringView, pair<StringView, StringView>> files; // test => (in, out)
    ...
    for (const string& str : tests_files.as_array()) {
        ...
        if (in_file.empty()) {
            throw std::runtime_error(concat_tostr(
                    "Simfile: `test_files`: missing input file for test `", test_name, '`'));
        }
        ...
        if (not interactive and out_file.empty()) {
            throw std::runtime_error(concat_tostr(
                    "Simfile: `test_files`: missing output file for test: `", test_name, '`'));
        }

        auto it = files.emplace(test_name, pair<StringView, StringView>(in_file, out_file));
        if (not it.second) {
            throw std::runtime_error{concat_tostr(
                    "Simfile: `test_files`: redefinition of the test `", test_name, '`')};
        }
    }
    ...
    for (TestGroup& group : tgroups) {
        for (Test& test : group.tests) {
            ...
            test.in = path_absolute(it->second.first).erase(0, 1); // Erase '/' from
                                                                   // the beginning
            if (interactive) {
                test.out = std::nullopt;
            } else {
                test.out = path_absolute(it->second.second).erase(0, 1); // Same here
            }
        }
    }
