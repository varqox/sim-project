...
public:
    explicit DirPackageLoader(FilePath pkg_path)
    : pkg_root_(pkg_path) {
...
int JudgeWorker::compile_impl(FilePath source, SolutionLanguage lang,
        ...
        size_t c_errors_max_len, const string& proot_path, StringView compilation_source_basename,
    ...
    int rc = compile(compilation_dir, compile_command(lang, src_filename, exec_dest_filename),
...
int JudgeWorker::compile_checker(std::optional<std::chrono::nanoseconds> time_limit,
    ...
    return compile_impl(checker_path_and_lang.first, checker_path_and_lang.second, time_limit,
...
Sandbox::ExitStat JudgeWorker::run_solution(FilePath input_file, FilePath output_file,
    ...
    if (es.si.code == CLD_EXITED and es.si.status == 0) {
        ...
        auto wrong_second_line = [&]() -> CheckerStatus {
            return {CheckerStatus::ERROR, 0,
                    concat_tostr("Second line of the ", output_name, " is invalid: `", line2,
        ...
        // * -> Checker error
        return {CheckerStatus::ERROR, 0,
                concat_tostr("First line of the ", output_name, " is invalid: `", line1,
    ...
    for (auto const& group : sf.tgroups) {
        ...
        for (auto const& test : group.tests) {
            if (skip_tests) {
                ...
                report_group.tests.emplace_back(test.name, JudgeReport::Test::SKIPPED, 0s,
                        test.time_limit, 0, test.memory_limit, string{});
            } else {
...
JudgeReport JudgeWorker::judge_interactive(bool final, JudgeLogger& judge_log,
    ...
    auto checker_supervisor = [&] {
        try {
            ...
            for (;;) {
                ...
                try {
                    // Run checker
                    ces = sandbox.run(checker_path, {checker_path, job.test_in_path}, opts,
    ...
    auto judge_on_test = [&](const sim::Simfile::Test& test, double& group_score_ratio) {
        ...
        next_job_promise.set_value(NextJob{std::move(checker_input), std::move(checker_output),
        ...
        JudgeReport::Test test_report(test.name, JudgeReport::Test::OK, es.cpu_runtime,
...
JudgeReport JudgeWorker::judge(bool final, JudgeLogger& judge_log,
    ...
    auto judge_on_test = [&](const sim::Simfile::Test& test, double& group_score_ratio) {
        ...
        // Run solution on the test
        Sandbox::ExitStat es = sandbox.run(solution_path, {},
                {test_in, solution_stdout, -1, cpu_time_limit_to_real_time_limit(test.time_limit),
                        test.memory_limit, test.time_limit}); // Allow exceptions to fly upper
        ...
        auto ces = sandbox.run(checker_path,
                {checker_path, test_in_path, test_out_path, sol_stdout_path}, checker_opts,
                {{test_in_path, OpenAccess::RDONLY}, {test_out_path, OpenAccess::RDONLY},
                        {sol_stdout_path, OpenAccess::RDONLY}}); // Allow exceptions to fly higher
