...
static_assert(
	std::is_same<
		decltype(SCMP_CMP(0, SCMP_CMP_MASKED_EQ, 0, 0)), scmp_arg_cmp>::value,
...
Sandbox::Sandbox() {
...

	{
		...
		seccomp_attr_set_throw(
			x86_ctx_, SCMP_FLTATR_ACT_BADARCH,
			SCMP_ACT_TRACE(invalid_arch_callback_id));
		seccomp_attr_set_throw(
			x86_64_ctx_, SCMP_FLTATR_ACT_BADARCH,
			SCMP_ACT_TRACE(invalid_arch_callback_id));
	}
	...
	public:
		SyscallCallbackLimiting(
			Sandbox& sandbox, uint limit, const char* syscall_name)
		: sandbox_(sandbox)
		, limit_(limit)
		, counter_(limit)
		...
		bool operator()() override {
			DEBUG_SANDBOX(stdlog(
				"limiting callback name: \"", syscall_name_,
				"\", current limit: ", counter_));
			if (counter_ == 0) {
				return sandbox_.set_message_callback(
					"forbidden syscall: ", syscall_name_);
			}
			...
			[](pid_t tracee_pid, auto& regs,
			   const vector<AllowedFile>& allowed_files) {
				...
				return check_opening_syscall(
					tracee_pid, regs, regs.syscall(), regs.res(), regs.arg2(),
		...
		seccomp_rule_add_throw(
			...
			SCMP_ACT_TRACE(add_callback(
				[&] {
					X86SyscallRegisters regs(tracee_pid_);
					return check_opening_syscall(
						tracee_pid_, regs, regs.syscall(), regs.res(),
	...
	if (tracee_pid_ == 0) { // Child = tracee
		...
		auto send_error_and_exit = [&](auto&&... args) {
			send_error_message_and_exit(
				pfd[1], std::forward<decltype(args)>(args)...);
		};
...
#define DISALLOW_ONLY_ON_STDIN_STDOUT_STDERR(arch_ctx, syscall, errno)     \
	...
	seccomp_rule_add_throw(                                                \
		...
		SCMP_SYS(syscall), 1, SCMP_A0(SCMP_CMP_EQ, STDERR_FILENO));        \
                                                                           \
	seccomp_rule_add_throw(                                                \
		arch_ctx, SCMP_ACT_ALLOW, SCMP_SYS(syscall), 1,                    \
...
#define DISALLOW_ONLY_ON_STDIN_STDOUT_STDERR_BOTH_CTX(syscall, errno) \
	...
	DISALLOW_ONLY_ON_STDIN_STDOUT_STDERR(x86_64_ctx_, syscall, errno)
			...
			DISALLOW_ONLY_ON_STDIN_STDOUT_STDERR(
				x86_ctx_, fadvise64_64, ESPIPE);
...
#undef DISALLOW_ONLY_ON_STDIN_BOTH_CTX
			...
			seccomp_rule_add_both_ctx(
				SCMP_ACT_ALLOW, SCMP_SYS(tgkill), 2,
				SCMP_A0(SCMP_CMP_EQ, tracee_pid_),
				SCMP_A1(SCMP_CMP_EQ, tracee_pid_));
			seccomp_rule_add_both_ctx(
				SCMP_ACT_ERRNO(EPERM), SCMP_SYS(tgkill), 1,
				SCMP_A0(SCMP_CMP_NE, tracee_pid_));
			seccomp_rule_add_both_ctx(
				SCMP_ACT_ERRNO(EPERM), SCMP_SYS(tgkill), 2,
				SCMP_A0(SCMP_CMP_EQ, tracee_pid_),
				SCMP_A1(SCMP_CMP_NE, tracee_pid_));
			// kill (allow killing the calling process only)
			seccomp_rule_add_both_ctx(
				SCMP_ACT_ALLOW, SCMP_SYS(kill), 1,
				SCMP_A0(SCMP_CMP_EQ, tracee_pid_));
			seccomp_rule_add_both_ctx(
				SCMP_ACT_ERRNO(EPERM), SCMP_SYS(kill), 1,
				SCMP_A0(SCMP_CMP_NE, tracee_pid_));
		...
		DEBUG_SANDBOX({
			...
			if (DEBUG_SANDBOX_LOG_BPF_FILTER) {
				FileDescriptor fd(
					"/tmp/sim-sandbox-filter.bpf", O_WRONLY | O_CREAT | O_TRUNC,
					S_0644);
				if (fd < 0)
...
#define DEBUG_SANDBOX_VERBOSE_LOG(...) \
	...
	rusage ru{};
	if (syscalls::waitid(
			P_PID, tracee_pid_, &si, WSTOPPED | WEXITED, nullptr) == -1)
	{
		THROW("waitid()", errmsg());
	}
	...
	if (si.si_code != CLD_TRAPPED) {
		return ExitStat(
			0ns, 0ns, si.si_code, si.si_status, ru, 0,
	...
	// virtual memory)
	tracee_statm_fd_.open(
		concat("/proc/", tracee_pid_, "/statm"), O_RDONLY | O_CLOEXEC);
	...
	auto get_cpu_time_and_wait_tracee = [&](bool is_waited = false) {
		...
		if (not is_waited) {
			syscalls::waitid(
				P_PID, tracee_pid_, &si, WEXITED | WNOWAIT, nullptr);
		}
	...
	try {
		for (;;) {
			...
			// Waiting for events
			syscalls::waitid(
				P_PID, tracee_pid_, &si, WSTOPPED | WEXITED | WNOWAIT, nullptr);
			...
			case CLD_TRAPPED:
				...
				if (has_tracee_timeouted()) {
					DEBUG_SANDBOX_VERBOSE_LOG(
						"TRAPPED (TIMEOUT) - si_status: ", si.si_status);
				...
				DEBUG_SANDBOX_VERBOSE_LOG(
					"TRAPPED (unknown) - si_status: ", si.si_status);
				...
				if (ptrace(PTRACE_CONT, tracee_pid_, 0, si.si_status)) {
					DEBUG_SANDBOX(stdlog(
						'[', tracee_pid_, "] ptrace(PTRACE_CONT)", errmsg()));
				}
			...
			case CLD_STOPPED:
				DEBUG_SANDBOX_VERBOSE_LOG(
					"STOPPED - si_status: ", si.si_status);
				// Deliver intercepted signal to tracee
				if (ptrace(PTRACE_CONT, tracee_pid_, 0, si.si_status)) {
					DEBUG_SANDBOX(stdlog(
						'[', tracee_pid_, "] ptrace(PTRACE_CONT)", errmsg()));
