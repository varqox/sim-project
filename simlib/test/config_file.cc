...
TEST(ConfigFile, is_string_literal) {
	...
	for (int a = 0; a < 256; ++a) {
		for (int b = 0; b < 256; ++b) {
			...
			EXPECT_EQ(
				ConfigFile::is_string_literal(StringView(t.data(), 4)), false)
				<< "a: " << a << " b: " << b
...
TEST(ConfigFile, escape_to_double_quoted_string) {
	...
	for (auto&& p : cases) {
		...
		// Second version
		EXPECT_EQ(
			ConfigFile::full_escape_to_double_quoted_string(p.first), p.second)
			<< "p.first: " << p.first << endl;
	}
	...
	EXPECT_EQ(
		ConfigFile::full_escape_to_double_quoted_string("ś"),
		R"===("\xc5\x9b")===");
	EXPECT_EQ(
...
TEST(ConfigFile, escape_string) {
	...
	// Escaping control characters
	EXPECT_EQ(
		ConfigFile::escape_string("\x07\x0e\n\x15\x1f"),
		R"===("\a\x0e\n\x15\x1f")===");
	EXPECT_EQ(
		ConfigFile::escape_string("\x02\x03\tśćąłź\x11"),
		R"===("\x02\x03\tśćąłź\x11")===");
...
workers: "2" # [103,104)
	...
	// use it here
	EXPECT_THROW(
		cf.load_config_from_string("a & eee "), ConfigFile::ParseError);
	EXPECT_THROW(cf.load_config_from_string("a = [a"), ConfigFile::ParseError);
	EXPECT_THROW(cf.load_config_from_string("a = 'a"), ConfigFile::ParseError);
	EXPECT_THROW(cf.load_config_from_string("a = \"a"), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = b\nb"), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = \"b\nb\""), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = 'b\nb'"), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = 'b'b'"), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = \"b\"b\""), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = \"\\q\""), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = a\n\n&"), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = a\n\na = '"), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = \"\\xg21\""), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = \"\\x2g1\""), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = \"\\x\""), ConfigFile::ParseError);
	EXPECT_THROW(
		cf.load_config_from_string("a = \"\\xa\""), ConfigFile::ParseError);
