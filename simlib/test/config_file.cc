...
TEST(ConfigFile, is_string_literal) {
    ...
    auto is_interior = [](auto c) { return not is_one_of(c, '\n', '#', ']', ','); };
    auto is_ending = [](auto c) { return not(is_space(c) || is_one_of(c, '#', ']', ',')); };
    auto dump = [](int a, int b = -1, int c = -1) {
        char t[3] = {static_cast<char>(a), static_cast<char>(b), static_cast<char>(c)};
    ...
    for (int a = 0; a < 256; t[0] = ++a) {
        ...
        for (int b = 0; b < 256; t[1] = ++b) {
            // Two characters
            EXPECT_EQ(
                ConfigFile::is_string_literal({t.data(), 2}), is_beginning(t[0]) && is_ending(t[1])
            ) << dump(a, b)
              << endl;
