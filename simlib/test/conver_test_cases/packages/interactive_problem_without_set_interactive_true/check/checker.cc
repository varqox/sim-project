...
int main(int argc, char** argv) {
	...
	ifstream test(argv[1]); // You can comment it out if you don't use it
	my_assert(test.is_open());
	Scanner scan(stdin, Scanner::Mode::IGNORE_WS_BEFORE_EOF);

	int n;
	scan >> integer(n, 0, 100);
	verdict.ok(n, "Solution decided how much it scores");
}
