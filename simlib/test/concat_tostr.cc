...
TEST(concat_tostr, concat_tostr) {
    ...
    EXPECT_EQ("ab c de", concat_tostr("a", 'b', ' ', CStringView("c "), string("de")));
    EXPECT_EQ(StringView("a\0\0abc", 6), concat_tostr('a', '\0', '\0', StringView("abc")));
...
static string concat_and_back_insert_impl(
    std::tuple<Args&&...> args, std::index_sequence<Idx1...> /*unused*/,
...
static string concat_and_back_insert(Args&... args) {
    return concat_and_back_insert_impl(
        std::forward_as_tuple(std::forward<Args>(args)...), meta::span<0, LEFT_ARGS_NO>(),
...
static void test_back_insert_on(int line, T&& expected, Args&&... args) {
    EXPECT_EQ(expected, concat_and_back_insert<LEFT_ARGS_NO>(args...)) << "line: " << line;
...
TEST(concat_tostr, back_insert) {
    ...
    test_back_insert_on(
        __LINE__, "abc true 0 1 -1 2 3 -3 false", "abc ", true, " ", 0, ' ', 1, ' ', -1, " ",
        2, ' ', 3, ' ', -3, ' ', false);
