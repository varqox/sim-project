...
TEST(create_unique_file, typical) {
    ...
    string path2 = path1;
    auto opt = create_unique_file(-1, path1.data(), path1.size(), 4, O_RDONLY | O_CLOEXEC, S_0644);
...
TEST(create_unique_file, fail) {
    ...
    auto opt = create_unique_file(-1, path.data(), path.size(), 0, O_RDONLY | O_CLOEXEC, S_0600);
