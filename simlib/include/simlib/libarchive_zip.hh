...
void skim_archive(int fd, Func&& setup_archive, UnaryFunc&& entry_callback) {
	...
	for (;;) {
		...
		if constexpr (std::is_convertible_v<
						  decltype(entry_callback(entry)), bool>) {
...
template <class Func, class UnaryFunc>
void skim_archive(
	FilePath filename, Func&& setup_archive, UnaryFunc&& entry_callback) {
	...
	return skim_archive(
		fd, std::forward<Func>(setup_archive),
...
void skim_zip(int fd, UnaryFunc&& entry_callback) {
	return skim_archive(
		fd, archive_read_support_format_zip,
		std::forward<UnaryFunc>(entry_callback));
}
...
void skim_zip(FilePath filename, UnaryFunc&& entry_callback) {
	return skim_archive(
		filename, archive_read_support_format_zip,
		std::forward<UnaryFunc>(entry_callback));
}
...
template <class Func, class UnaryFunc>
void extract(
	int archive_fd, int flags, Func&& setup_archives, UnaryFunc&& extract_entry,
	StringView dest_dir = ".") {
	...
	for (;;) {
		...
		if (archive_entry_size(entry) > 0) {
			...
			char buff[BUFF_SIZE];
			decltype(archive_read_data(in, buff, BUFF_SIZE)) rr = 0;
...
template <class Func, class UnaryFunc>
void extract(
	FilePath filename, int flags, Func&& setup_archives,
	...
	return extract(
		fd, flags, std::forward<Func>(setup_archives),
...
template <class Func>
std::string
extract_file(int archive_fd, Func&& setup_archive, const StringView& pathname) {
	struct archive* in = archive_read_new();
	throw_assert(in);
	Defer in_guard([&] { archive_read_free(in); });
...
template <class Func>
std::string
extract_file(FilePath archive_file, Func&& setup_archive, StringView pathname) {
...
inline std::string
extract_file_from_zip(int zip_fd, const StringView& pathname) {
	return extract_file(zip_fd, archive_read_support_format_zip, pathname);
}

/// Extract @p pathname from the zip archive @p zip_file
inline std::string
extract_file_from_zip(FilePath zip_file, StringView pathname) {
...
inline void extract_zip(
	int zip_fd, int flags, UnaryFunc&& extract_entry,
	StringView dest_dir = ".") {
	return extract(
		zip_fd, flags,
		[](archive* in, archive* /*unused*/) {
...
template <class UnaryFunc>
inline void extract_zip(
	FilePath filename, int flags, UnaryFunc&& extract_entry,
	StringView dest_dir = ".") {
	return extract(
		filename, flags,
		[](archive* in, archive* /*unused*/) {
...
/// Extracts zip, for details see extract() documentation
inline void extract_zip(
	FilePath filename, int flags = ARCHIVE_EXTRACT_TIME,
	StringView dest_dir = ".") {
	return extract_zip(
...
void compress(
	Container&& filenames, FilePath archive_filename, Func&& setup_archive) {
	...
	FileDescriptor fd(
		archive_filename, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC);
	if (fd == -1) {
	...
	auto impl = [&](auto&& self, struct stat64& st) -> void {
		...
		// Directory
		{
			...
			archive_entry_set_atime(
				entry, st.st_atim.tv_sec, st.st_atim.tv_nsec);
			archive_entry_set_ctime(
				entry, st.st_ctim.tv_sec, st.st_ctim.tv_nsec);
			archive_entry_set_mtime(
				entry, st.st_mtim.tv_sec, st.st_mtim.tv_nsec);
...
inline void compress(
	std::initializer_list<T> filenames, FilePath archive_filename,
	Func&& setup_archive) {
...
void compress_into_zip(Container&& filenames, FilePath zip_archive_filename) {
	return compress(
		std::forward<Container>(filenames), zip_archive_filename,
