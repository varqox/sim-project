...
void skim_archive(int fd, Func&& setup_archive, UnaryFunc&& entry_callback) {
    ...
    for (;;) {
        ...
        if constexpr (std::is_convertible_v<decltype(entry_callback(entry)), bool>) {
 ...
 */
template <class Func, class UnaryFunc>
void skim_archive(FilePath filename, Func&& setup_archive, UnaryFunc&& entry_callback) {
...
template <class Func, class UnaryFunc>
void extract(
    FilePath filename, int flags, Func&& setup_archives, UnaryFunc&& extract_entry,
    StringView dest_dir = ".") {
    FileDescriptor fd{filename, O_RDONLY | O_CLOEXEC};
    if (fd == -1) {
        THROW("Failed to open file `", filename, '`', errmsg());
    ...
    return extract(
        fd, flags, std::forward<Func>(setup_archives), std::forward<UnaryFunc>(extract_entry),
        dest_dir);
}
 ...
 */
template <class Func>
std::string extract_file(int archive_fd, Func&& setup_archive, const StringView& pathname) {
    struct archive* in = archive_read_new();
    throw_assert(in);
    Defer in_guard([&] { archive_read_free(in); });
 ...
 */
template <class Func>
std::string extract_file(FilePath archive_file, Func&& setup_archive, StringView pathname) {
...
inline std::string extract_file_from_zip(int zip_fd, const StringView& pathname) {
    return extract_file(zip_fd, archive_read_support_format_zip, pathname);
}

/// Extract @p pathname from the zip archive @p zip_file
inline std::string extract_file_from_zip(FilePath zip_file, StringView pathname) {
...
/// Extracts zip, for details see extract() documentation
inline void
extract_zip(int zip_fd, int flags = ARCHIVE_EXTRACT_TIME, StringView dest_dir = ".") {
    return extract_zip(
...
void compress(Container&& filenames, FilePath archive_filename, Func&& setup_archive) {
...
template <class T>
inline void
compress_into_zip(std::initializer_list<T> filenames, FilePath zip_archive_filename) {
