...
http://keccak.noekeon.org/specs_summary.html This file uses UTF-8 encoding, as
some comments use Greek letters.
================================================================
*/
...
#define writeLane(x, y, lane)                                                  \
	store64((UINT8*)state + sizeof(tKeccakLane) * i(x, y), lane)
#define XORLane(x, y, lane)                                                    \
	xor64((UINT8*)state + sizeof(tKeccakLane) * i(x, y), lane)
#endif
...
void KeccakF1600_StatePermute(void* state) {
	...
	for (round = 0; round < 24; round++) {
		...
		{ // === ρ and π steps (see [Keccak Reference, Sections 2.3.3
		  // and 2.3.4]) ===
