...
Response do_list(Context& ctx, uint32_t limit, sql::Condition<Params...> where_cond) {
    ...
    auto tags_stmt =
        ctx.mysql.prepare_bind_and_execute(sql::Select("pt.problem_id, pt.name, pt.is_hidden")
...
caps_to_condition(ProblemsListCapabilities caps, optional<decltype(Problem::type)> problem_type) {
        ...
        (!problem_type or problem_type == Problem::Type::PRIVATE))
    {
        res = res or
            sql::Condition(
                  concat_tostr("p.type=", decltype(Problem::type){Problem::Type::PRIVATE}.to_int())
            );
    }
    return res;
}
...
template <class... Params>
static Response do_list_user_problems(
    Context& ctx,
    uint32_t limit,
