...
public:
    explicit TestIdIterator(
        ...
        std::map<uint64_t, sim::sql_fields::Datetime> id_to_datetime,
    ...
    [[nodiscard]] std::optional<sim::sql_fields::Datetime> created_at_upper_bound_of_id(uint64_t id
    ) override {
    ...
    for (size_t rep = 0; rep < REPS; ++rep) {
        ...
        try_again:
            ...
            auto int_to_datetime = [](int x) {
                assert(0 <= x && x < 1000);
                return sim::sql_fields::Datetime{concat_tostr(x + 2000, "-01-01 00:00:00")};
            };
            ...
            std::map<uint64_t, sim::sql_fields::Datetime> current_id_to_datetime;
            ...
            std::map<uint64_t, sim::sql_fields::Datetime> other_id_to_datetime;
            ...
            if (::testing::Test::HasFailure()) {
                cerr << "min_id: " << min_id << endl;
                cerr << "max_id_plus_one: " << max_id_plus_one << endl;
                cerr << "ids: {" << endl;
                for (size_t i = 0; i < max_id_plus_one - min_id; ++i) {
                    cerr << "  " << std::setw(3) << i + min_id << ' '
                         << int_to_datetime(created_at[i]).to_string() << " " << std::setw(7)
                         << (comes_from_current.get(i) ? "current" : "other") << ' '
                         << (is_deleted.get(i) ? "deleted" : "") << endl;
                }
                cerr << "}" << endl;
                cerr << "current (min_id = " << current_min_id
                     << " max_id_plus_one = " << current_max_id_plus_one << "): {" << endl;
                ...
                for (size_t id = current_min_id; id < current_max_id_plus_one; ++id) {
                    ...
                    auto datetime_str = [&] {
                        if (current_ids_it != current_ids.end() && current_ids_it->id == id) {
                            deleted = false;
                            return (current_ids_it++)->created_at.to_string();
                        }
                    ...
                    cerr << "  " << std::setw(3) << id << ' ' << std::setw(19) << datetime_str
                         ...
                         << " (merge tells: " << merged_ids.current_id_to_new_id(id) << ")" << endl;
                }
                cerr << "}" << endl;
                cerr << "other (min_id = " << other_min_id
                     << " max_id_plus_one = " << other_max_id_plus_one << "): {" << endl;
                ...
                for (size_t id = other_min_id; id < other_max_id_plus_one; ++id) {
                    ...
                    auto datetime_str = [&] {
                        if (other_ids_it != other_ids.end() && other_ids_it->id == id) {
                            deleted = false;
                            return (other_ids_it++)->created_at.to_string();
                        }
                    ...
                    cerr << "  " << std::setw(3) << id << ' ' << std::setw(19) << datetime_str
                         ...
                         << " (merge tells: " << merged_ids.other_id_to_new_id(id) << ")" << endl;
                }
                cerr << "}" << endl;
                break;
            }
