...
TEST(strongly_typed_function, constructor_checks_arguments) {
    ...
    static_assert(not std::
                      is_constructible_v<strongly_typed_function<void(int)>, decltype(uint_foo)>);
    static_assert(not std::is_constructible_v<
                  strongly_typed_function<void(int)>,
                  decltype(uint_noexcept_foo)>);

    auto lambda = [&](uint /*unused*/) {};
    static_assert(not std::
                      is_constructible_v<strongly_typed_function<void(int)>, decltype(lambda)>);
    auto lambda_mut = [&](uint /*unused*/) mutable {};
    static_assert(not std::
                      is_constructible_v<strongly_typed_function<void(int)>, decltype(lambda_mut)>);
}
...
TEST(strongly_typed_function, constructor_checks_return_value) {
    ...
    static_assert(not std::
                      is_constructible_v<strongly_typed_function<int()>, decltype(lambda_mut)>);
}

// NOLINTNEXTLINE
TEST(strongly_typed_function, call) {
    strongly_typed_function<int64_t(int64_t, int32_t)> func([](int64_t x, int32_t y) {
        return x - y;
    });
