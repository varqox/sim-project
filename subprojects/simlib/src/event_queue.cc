...
EventQueue::EventQueue() : immediate_pause_fd_(eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK)) {
...
handler_id_t EventQueue::add_time_handler_impl(time_point tp, std::function<void()> handler) {
...
void EventQueue::run() {
    ...
    auto process_timed_handlers = [&] {
        ...
        do {
            ...
            auto handler = std::move(std::get<TimedHandler>(it->second).handler);
            handlers_.erase(it);
        ...
        } while ((now = std::chrono::system_clock::now()) <= start + TIME_QUANTUM);
    };
    ...
    while (handlers_.size() > 1 and not immediate_pause_was_requested()) {
        ...
        const int ready_poll_events_num = [&] {
            ...
            return ppoll(poll_events_.data(), poll_events_.size(), tmo_p, nullptr);
        }();
