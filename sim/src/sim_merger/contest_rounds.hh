...
class ContestRoundsMerger : public Merger<sim::contest_rounds::ContestRound> {
    ...
    void load(RecordSet& record_set) override {
        ...
        auto stmt = conn.prepare("SELECT cr.id, cr.contest_id, cr.name, cr.item,"
                                 " cr.begins, cr.ends, cr.full_results,"
                                 " cr.ranking_exposure, MIN(s.submit_time) "
                                 "FROM ",
                record_set.sql_table_name, " cr LEFT JOIN ", record_set.sql_table_prefix,
                "submissions s "
                "ON s.contest_round_id=cr.id GROUP BY cr.id");
        stmt.bind_and_execute();
        stmt.res_bind_all(cr.id, cr.contest_id, cr.name, cr.item, cr.begins, cr.ends,
                cr.full_results, cr.ranking_exposure, earliest_submit_time);
...
public:
    void save_merged() override {
        ...
        for (const NewRecord& new_record : new_table_) {
            ...
            stmt.bind_and_execute(x.id, x.contest_id, x.name, x.item, x.begins, x.ends,
                    x.full_results, x.ranking_exposure);
        }
