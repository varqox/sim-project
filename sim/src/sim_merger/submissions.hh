...
class SubmissionsMerger : public Merger<sim::submissions::Submission> {
    ...
    void load(RecordSet& record_set) override {
        ...
        auto stmt = conn.prepare("SELECT id, file_id, owner, problem_id,"
        ...
        stmt.res_bind_all(s.id, s.file_id, m_owner, s.problem_id, m_contest_problem_id,
                m_contest_round_id, m_contest_id, s.type, s.language, s.final_candidate,
                s.problem_final, s.contest_final, s.contest_initial_final, s.initial_status,
                s.full_status, s.submit_time, m_score, s.last_judgment, s.initial_report,
                s.final_report);
        while (stmt.next()) {
