...
class ContestProblemsMerger : public Merger<sim::contest_problems::ContestProblem> {
    ...
    void load(RecordSet& record_set) override {
        ...
        auto stmt = conn.prepare("SELECT cp.id, cp.contest_round_id, cp.contest_id,"
...
public:
    void save_merged() override {
        ...
        for (const NewRecord& new_record : new_table_) {
            ...
            stmt.bind_and_execute(x.id, x.contest_round_id, x.contest_id, x.problem_id, x.name,
                    x.item, x.method_of_choosing_final_submission, x.score_revealing);
        }
