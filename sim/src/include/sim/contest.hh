...
get(MySQL::Connection& mysql, GetIdKind id_kind, T&& id,
	...
	case GetIdKind::CONTEST_ROUND: {
		...
		id_part_sql =
		   "JOIN contest_rounds cr ON cr.id=? AND cr.contest_id=c.id";
		break;
	}
	case GetIdKind::CONTEST_PROBLEM: {
		...
		fields.append(", cr.begins");
		id_part_sql =
		   "JOIN contest_problems cp ON cp.id=? AND cp.contest_id=c.id "
