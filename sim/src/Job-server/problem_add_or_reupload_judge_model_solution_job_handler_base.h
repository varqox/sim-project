...
class ProblemAddOrReuploadJudgeModelSolutionJobHandlerBase
	: public ProblemPackageResetLimitsJobHandlerBase, public ProblemAddOrReuploadJobHandlerBase {
protected:
	ProblemAddOrReuploadJudgeModelSolutionJobHandlerBase(uint64_t job_id,
			JobType job_type, StringView job_creator,
			const jobs::AddProblemInfo& info, uint64_t job_file_id,
			Optional<uint64_t> tmp_file_id, Optional<uint64_t> problem_id)
		: JobHandler(job_id), ProblemAddOrReuploadJobHandlerBase(job_type,
