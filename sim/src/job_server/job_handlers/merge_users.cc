...
void MergeUsers::run_impl() {
	...
	// Transfer contest_users
	{
		...
		mysql
		   .prepare("UPDATE IGNORE contest_users SET user_id=? WHERE user_id=?")
		   .bind_and_execute(info_.target_user_id, donor_user_id_);
	}
	...
	{
		auto stmt =
		   mysql.prepare("SELECT DISTINCT problem_id, contest_problem_id FROM "
	...
	for (auto const& ftu_elem : finals_to_update) {
		submission::update_final_lock(mysql, info_.target_user_id,
		                              ftu_elem.problem_id);
		submission::update_final(mysql, info_.target_user_id,
		                         ftu_elem.problem_id,
	...
	mysql.prepare("DELETE FROM users WHERE id=?")
	   .bind_and_execute(donor_user_id_);
