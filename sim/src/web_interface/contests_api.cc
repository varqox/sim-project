...
static void append_contest_actions_str(
    T& str, sim::contest::OverallPermissions overall_perms, sim::contest::Permissions perms) {
...
public:
    ContestInfoResponseBuilder(
        ContentType& resp_content, sim::contest::OverallPermissions contest_overall_perms,
        ...
        auto cumode =
            (res.is_null(USER_MODE)
                 ...
                 : optional(ContestUser::Mode(WONT_THROW(
                       str2num<std::underlying_type_t<ContestUser::Mode>>(res[USER_MODE])
                           .value()))));
        auto contest_perms = sim::contest::get_permissions(
            (session_is_open ? optional(session_user_type) : std::nullopt), is_public, cumode);
...
void Sim::api_contest() {
    ...
    sim::contest_round::iterate(
        mysql, sim::contest_round::IterateIdKind::CONTEST, contest_id, contest_perms,
        curr_date, [&](const ContestRound& round) { resp_builder.append_round(round); });
...
void Sim::api_contest_round(StringView contest_round_id) {
    ...
    sim::contest_round::iterate(
        mysql, sim::contest_round::IterateIdKind::CONTEST_ROUND, contest_round_id,
        contest_perms, curr_date, [&](const ContestRound& cr) { contest_round_opt = cr; });
...
void Sim::api_contest_problem(StringView contest_problem_id) {
    ...
    sim::contest_round::iterate(
        mysql, sim::contest_round::IterateIdKind::CONTEST_PROBLEM, contest_problem_id,
...
void Sim::api_contest_create(sim::contest::OverallPermissions overall_perms) {
    ...
    mysql
        .prepare("INSERT contest_users(user_id, contest_id, mode) "
                 "VALUES(?, ?, ?)")
        .bind_and_execute(session_user_id, contest_id, EnumVal(ContestUser::Mode::OWNER));
...
void Sim::api_contest_clone(sim::contest::OverallPermissions overall_perms) {
    ...
    sim::contest_problem::iterate(
        ...
        [&](const ContestProblem& cp,
            ...
            if (uint(~extra_data.problem_perms & sim::problem::Permissions::VIEW)) {
    ...
    for (auto& [key, r] : contest_rounds) {
        stmt.bind_and_execute(
            r.contest_id, r.name, r.item, r.begins, r.ends, r.full_results,
            r.ranking_exposure);
    ...
    mysql
        .prepare("INSERT contest_users(user_id, contest_id, mode) "
                 "VALUES(?, ?, ?)")
        .bind_and_execute(
            session_user_id, new_contest_id, EnumVal(sim::ContestUser::Mode::OWNER));
...
void Sim::api_contest_edit(
    StringView contest_id, sim::contest::Permissions perms, bool is_public) {
    STACK_UNWINDING_MARK;
    ...
    if (make_submitters_contestants) {
        mysql
            ...
            .bind_and_execute(
                contest_id, EnumVal(sim::ContestUser::Mode::CONTESTANT), contest_id);
    }

    transaction.commit();
}

void Sim::api_contest_delete(StringView contest_id, sim::contest::Permissions perms) {
    STACK_UNWINDING_MARK;
...
void Sim::api_contest_round_create(StringView contest_id, sim::contest::Permissions perms) {
    STACK_UNWINDING_MARK;
...
void Sim::api_contest_round_clone(StringView contest_id, sim::contest::Permissions perms) {
    STACK_UNWINDING_MARK;
    ...
    optional<ContestRound> contest_round_opt;
    sim::contest_round::iterate(
        mysql, sim::contest_round::IterateIdKind::CONTEST_ROUND, source_contest_round_id,
        source_contest_perms, curr_date,
    ...
    auto stmt = mysql.prepare("INSERT contest_rounds(contest_id, name, item, begins, ends,"
                              ...
                              "FROM contest_rounds WHERE contest_id=?");
    stmt.bind_and_execute(
        contest_id, contest_round.name, contest_round.begins, contest_round.ends,
...
void Sim::api_contest_round_edit(uintmax_t contest_round_id, sim::contest::Permissions perms) {
    STACK_UNWINDING_MARK;
    ...
    auto stmt = mysql.prepare("UPDATE contest_rounds "
                              ...
                              "WHERE id=?");
    stmt.bind_and_execute(
        name, inf_timestamp_to_InfDatetime(begins).to_str(),
        inf_timestamp_to_InfDatetime(ends).to_str(),
        inf_timestamp_to_InfDatetime(full_results).to_str(),
...
void Sim::api_contest_round_delete(
    uintmax_t contest_round_id, sim::contest::Permissions perms) {
    STACK_UNWINDING_MARK;
...
void Sim::api_contest_problem_add(
    uintmax_t contest_id, uintmax_t contest_round_id, sim::contest::Permissions perms) {
    STACK_UNWINDING_MARK;
...
void Sim::api_contest_problem_rejudge_all_submissions(
    StringView contest_problem_id, sim::contest::Permissions perms, StringView problem_id) {
    STACK_UNWINDING_MARK;
    ...
    mysql
        ...
        .bind_and_execute(
            ...
            mysql_date(), jobs::dump_string(problem_id), contest_problem_id);
...
void Sim::api_contest_ranking(
    sim::contest::Permissions perms, StringView submissions_query_id_name,
    StringView query_id) {
    STACK_UNWINDING_MARK;
    ...
    auto prepare_stmt = [&](auto&& extra_cr_sql, auto&&... extra_bind_params) {
        ...
        stmt.res_bind_all(
            cr_id, cr_full_results, cp_id, cp_score_revealing, s_owner, sf_id, sf_full_status,
            sf_score, si_id, si_initial_status);
    };
