...
void Sim::api_contest_entry_token() {
    ...
    if (next_arg[0] == '=') {
        ...
        static_assert(
            CONTEST_ENTRY_TOKEN_LEN != CONTEST_ENTRY_SHORT_TOKEN_LEN,
        ...
        if (uint(~cperms & sim::contest::Permissions::MANAGE_CONTEST_ENTRY_TOKEN)) {
...
void Sim::api_contest_entry_token_use_to_enter_contest(uintmax_t contest_id) {
    ...
    stmt.bind_and_execute(
        session_user_id, contest_id, EnumVal(sim::ContestUser::Mode::CONTESTANT));
