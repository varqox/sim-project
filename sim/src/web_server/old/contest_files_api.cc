...
void Sim::api_contest_file_download(
    ...
    InplaceBuff<sim::FILE_NAME_MAX_LEN> filename;
    ...
    resp.content = sim::internal_file_path(internal_file_id);
}
...
void Sim::api_contest_file_add() {
    ...
    form_validate(name, "name", "File name", sim::FILE_NAME_MAX_LEN);
    form_validate(description, "description", "Description", sim::FILE_DESCRIPTION_MAX_LEN);
    ...
    if (name.empty()) {
        if (user_filename.size() > sim::FILE_NAME_MAX_LEN) {
            add_notification(
                "error", "Filename of the provided file is longer than ",
                sim::FILE_NAME_MAX_LEN, ". You have to provide a shorter name.");
        } else {
            name = user_filename;
        }
    }
    ...
    if (file_size > sim::FILE_MAX_SIZE) {
        add_notification(
            "error", "File is too big (maximum allowed size: ", sim::FILE_MAX_SIZE,
            " bytes = ", humanize_file_size(sim::FILE_MAX_SIZE), ')');
    ...
    CallInDtor internal_file_remover(
        [internal_file_id] { (void)unlink(sim::internal_file_path(internal_file_id)); });
    ...
    InplaceBuff<sim::FILE_ID_LEN> file_id;
    auto curr_date = mysql_date();
    throw_assert(session_is_open);

    do {
        file_id = sim::generate_random_token(sim::FILE_ID_LEN);
    ...
    // Move file
    if (move(file_tmp_path, sim::internal_file_path(internal_file_id))) {
        THROW("move()", errmsg());
    }
...
void Sim::api_contest_file_edit(
    ...
    CallInDtor internal_file_remover([internal_file_id] {
        if (internal_file_id > 0) {
            (void)unlink(sim::internal_file_path(internal_file_id));
        }
    });
