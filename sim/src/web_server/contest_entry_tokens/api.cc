...
std::optional<TokenContestInfo>
token_contest_info_for(Context& ctx, StringView token_or_short_token) {
    ...
    static_assert(
        ...
        "These cannot be equal because this would cause conflict in selecting the token in "
        "the below query");
    auto token_kind =
    ...
    stmt.bind_and_execute(
        ctx.session ? std::optional{ctx.session->user_id} : std::nullopt, token_or_short_token,
        token_or_short_token, mysql_date());
...
static void
set_or_regen_short_token(mysql::Connection& mysql, decltype(Contest::id) contest_id) {
    auto stmt = mysql.prepare(
        "UPDATE IGNORE contest_entry_tokens SET short_token=?, short_token_expiration=? WHERE "
