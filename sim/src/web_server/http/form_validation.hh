...
template <class T>
std::enable_if_t<std::is_integral_v<T> and !std::is_same_v<T, bool>, std::optional<T>> validate(
        ApiParam<T> api_param, CStringView str_val, std::string& errors_str) {
...
validate(ApiParam<sim::sql_fields::SatisfyingPredicate<SqlField, predicate_func, description_str>>
                 api_param,
...
// - !condition and field not set -> Some(None)
template <class T>
auto validate_required_and_allowed_only_if(ApiParam<T> api_param, const FormFields& form_fields,
        std::string& errors_str, bool condition) {
    const auto str_val_opt = form_fields.get(api_param.name);
    auto do_validate = [&] { return detail::validate(api_param, *str_val_opt, errors_str); };
    using ResT = std::optional<decltype(do_validate())>;
