...
Response sign_up(Context& ctx) {
    ...
    auto stmt =
        ctx.mysql.prepare("INSERT IGNORE INTO users(created_at, type, username, first_name, last_name, email, "
                          "password_salt, password_hash) VALUES(?, ?, ?, ?, ?, ?, ?, ?)");
    stmt.bind_and_execute(
        mysql_date(), user_type, username, first_name, last_name, email, password_salt, password_hash
    );
