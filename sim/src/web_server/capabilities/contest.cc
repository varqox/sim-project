        ...
        .node =
            {
                ...
                .delete_ = is_admin or contest_user_mode == ContestUser::Mode::OWNER,
...
contest_for(
    mysql::Connection& mysql, const decltype(web_worker::Context::session)& session,
    ...
    if (session) {
        stmt =
            mysql.prepare("SELECT c.is_public, cu.mode FROM contests c "
