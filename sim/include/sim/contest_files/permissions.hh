...
std::optional<std::pair<Permissions, OverallPermissions>> get_permissions(
        mysql::Connection& mysql, T&& contest_file_id, std::optional<U> user_id) {
    ...
    auto stmt = mysql.prepare("SELECT c.is_public", (user_id ? ", cu.mode, u.type " : " "),
