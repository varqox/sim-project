...
void Sipfile::load_static_tests() {
    ...
    for (StringView entry : static_tests_var.as_array()) {
        ...
        if (not entry.empty()) {
            log_warning(
                "Sipfile (static): ignoring invalid suffix: `",
                entry,
                "` of the entry with test range: ",
                test_range
            );
        }
...
void Sipfile::load_gen_tests() {
    ...
    for (StringView entry : gen_tests_var.as_array()) {
        ...
        for_each_test_in_range(test_range, [&](StringView test) {
            ...
            if (not inserted) {
                throw SipError(
                    "Sipfile (gen): test `", test, "` is specified in more than one test range"
                );
            }
        });
    }
