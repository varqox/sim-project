...
void restart_job(MySQL::Connection& mysql, StringView job_id, JobType job_type,
	...
	bool adding =
	   isOneOf(job_type, JT::ADD_PROBLEM, JT::ADD_JUDGE_MODEL_SOLUTION);
	...
	if (adding or reupload) {
		...
		// Restart job
		mysql
		   .prepare("UPDATE jobs SET type=?, status=" JSTATUS_PENDING_STR
		            ", tmp_file_id=NULL, info=? WHERE id=?")
		   .bindAndExecute(
		...
		mysql
		   .prepare("UPDATE jobs SET status=" JSTATUS_PENDING_STR " WHERE id=?")
		   .bindAndExecute(job_id);
	}
