	...
	Optional<uint64_t> aux_id, StringView info, StringView added)
{
	...
	try {
		...
		switch (jtype) {
		case JT::ADD_PROBLEM:
			job_handler = make_unique<ProblemAddJobHandler>(job_id,
				creator.value(), info, file_id.value(), tmp_file_id);
			break;

		case JT::REUPLOAD_PROBLEM:
			job_handler = make_unique<ProblemReuploadJobHandler>(job_id,
		...
		case JT::DELETE_PROBLEM:
			job_handler = make_unique<DeleteProblemJobHandler>(job_id,
				aux_id.value());
			break;

		case JT::MERGE_PROBLEMS:
			job_handler = make_unique<MergeProblemsJobHandler>(job_id,
				aux_id.value(), jobs::MergeProblemsInfo(info));
			break;

		case JT::DELETE_USER:
			job_handler = make_unique<DeleteUserJobHandler>(job_id,
				aux_id.value());
			break;
		...
		case JT::DELETE_CONTEST:
			job_handler = make_unique<DeleteContestJobHandler>(job_id,
				aux_id.value());
			break;

		case JT::DELETE_CONTEST_ROUND:
			job_handler = make_unique<DeleteContestRoundJobHandler>(job_id,
				aux_id.value());
			break;

		case JT::DELETE_CONTEST_PROBLEM:
			job_handler = make_unique<DeleteContestProblemJobHandler>(job_id,
				aux_id.value());
			break;
		...
		case JT::REJUDGE_SUBMISSION:
			job_handler = make_unique<JudgeOrRejudgeJobHandler>(job_id, aux_id.value(), added);
			break;
		...
		case JT::REUPLOAD_JUDGE_MODEL_SOLUTION:
			job_handler = make_unique<ProblemReuploadJudgeModelSolutionJobHandler>(
		...
		stmt.bindAndExecute(priority(JobType::DELETE_FILE), mysql_date(), job_id);

		// Fail job
		stmt = mysql.prepare("UPDATE jobs"
			" SET tmp_file_id=NULL, status=" JSTATUS_FAILED_STR ", data=? WHERE id=?");
		if (job_handler) {
			stmt.bindAndExecute(concat(job_handler->get_log(),
